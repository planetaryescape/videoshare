...
const uploadProgress = (h: Html, model: Model) => {
  ...
  return h.div([h.Class('mt-4')], [
    h.div([h.Class('mb-1 flex justify-between text-xs text-gray-400')], [
      stageLabel(model.uploadStage, model.uploadPct),
      indeterminate ? '' : `${model.uploadPct}%`,
    ]),
    h.div([h.Class('h-2 w-full overflow-hidden rounded-full bg-gray-700')], [
      h.div(
        [
          h.Class(
...
const chaptersSection = (h: Html, model: Model) =>
  h.div([h.Class('rounded-lg border border-gray-800 bg-gray-900/50 p-4')], [
    h.div([h.Class('mb-3 flex items-center justify-between')], [
    ...
    ...(model.editChapters.length === 0
      ...
      : model.editChapters.map((chapter) =>
        h.div([h.Class('mb-2 flex items-center gap-2')], [
          h.input([
            h.Class(
      ...
      ...(!video || !video.hlsKey
        ? [
          h.div([h.Class('rounded-lg border border-dashed border-gray-600 bg-gray-900/50 p-6')], [
            ...
            h.input([
              h.Class('block text-sm text-gray-400 file:mr-3 file:rounded-lg file:border-0 file:bg-gray-700 file:px-3 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-gray-600'),
              h.Type('file'),
            ...
            ...(model.selectedFile
              ? [
                h.div(
                  ...
                  ['Selected: ', h.span([h.Class('font-medium text-white')], [model.selectedFile.name])],
                ),
              ]
              : []),
            h.button(
