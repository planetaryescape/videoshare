...
export const LoadVideos = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedLoadVideos({ error: errMsg(error) }))),
  ),
)
...
export const CreateVideoCmd = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedCreateVideo({ error: errMsg(error) }))),
  ),
)
...
export const SaveVideoCmd = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedSaveVideo({ error: errMsg(error) }))),
  ),
)
...
export const LoadVideoDetail = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedLoadVideoDetail({ error: errMsg(error) }))),
  ),
)
...
export const SaveChaptersCmd = Command.define(
  ...
  SucceededSaveChapters,
  FailedSaveChapters,
)((input: { id: string; chapters: readonly Chapter[] }) =>
  Effect.gen(function* () {
    const response = yield* Effect.promise<Response>(() =>
      fetch(`/api/videos/${input.id}`, {
        method: 'PUT',
    ...
    Effect.catch((error) => Effect.succeed(FailedSaveChapters({ error: errMsg(error) }))),
  ),
)
...
export const UploadVideoCmd = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedUpload({ error: errMsg(error) }))),
  ),
)
...
export const PublishVideoCmd = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedPublish({ error: errMsg(error) }))),
  ),
)
...
export const DeleteVideoCmd = Command.define(
  ...
  Effect.gen(function* () {
    ...
    Effect.catch((error) => Effect.succeed(FailedDeleteVideo({ error: errMsg(error) }))),
  ),
)
