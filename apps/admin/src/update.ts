...
const saveChaptersCmd = (model: Model, chapters: readonly Chapter[]): ReadonlyArray<Cmd> => {
  ...
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      ...
      BlurredEditField: () => {
        ...
        const unchanged =
          model.editTitle === video.title &&
          model.editDescription === (video.description ?? '')
        if (unchanged || model.editTitle.trim() === '') {
          return [model, []] as const
        }
        return [
          model,
          [SaveVideoCmd({ id: video.id, title: model.editTitle, description: model.editDescription })],
        ] as const
      },
      ...
      FailedDeleteVideo: (msg: { error: string }) =>
        [evo(model, { errorMessage: () => Option.some(msg.error) }), []] as const,
      SucceededLoadVideoDetail: (msg: { video: Video; chapters: readonly Chapter[] }) =>
        [
          evo(model, {
            editVideo: () => Option.some(msg.video),
            editTitle: () => msg.video.title,
      ...
      BlurredChapterField: () =>
        [model, saveChaptersCmd(model, model.editChapters)] as const,
      SucceededSaveChapters: (msg: { chapters: readonly Chapter[] }) =>
        [evo(model, { editChapters: () => msg.chapters }), []] as const,
      FailedSaveChapters: (msg: { error: string }) =>
        [evo(model, { errorMessage: () => Option.some(msg.error) }), []] as const,
      ClickedCopyLink: (msg: { url: string }) =>
        [model, [CopyLinkCmd({ url: msg.url })]] as const,
